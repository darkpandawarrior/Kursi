...
fun WireAction.toEngine(): Action =
    when (this) {
...
fun ReactionStep.toWire(): WireReactionStep =
    when (this) {
...
fun WireReactionStep.toEngine(): ReactionStep =
    when (this) {
...
fun LossReason.toWire(): WireLossReason =
    when (this) {
...
fun WireLossReason.toEngine(): LossReason =
    when (this) {
...
fun PhaseView.toWire(): WirePhaseView =
    when (this) {
...
fun OpponentView.toWire(): WireOpponentView =
    WireOpponentView(
        id = id.raw,
        seatIndex = seatIndex,
        coins = coins,
...
fun GameConfig.toWire(): WireGameConfig =
    WireGameConfig(
        seatCount = seatCount,
 ...
 */
fun PlayerView.toWire(): WirePlayerView =
    WirePlayerView(
        viewer = viewer.raw,
 ...
 */
fun WireIntent.toEngine(): Intent =
    when (this) {
 ...
 */
fun GameEvent.toWireFor(viewer: PlayerId): WireGameEvent =
    when (this) {
