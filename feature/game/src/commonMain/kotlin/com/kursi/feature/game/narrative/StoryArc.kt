...
data class SeatRef(val seat: Int, val name: String)
...
sealed interface SocialOp {
    data class Ally(val a: Int, val b: Int) : SocialOp
    data class Betray(val seat: Int) : SocialOp
    data class Threat(val seat: Int, val delta: Float) : SocialOp
    data class Suspicion(val observer: Int, val target: Int, val delta: Float) : SocialOp
    data class Trust(val observer: Int, val target: Int, val delta: Float) : SocialOp
    data class Agitate(val seat: Int, val flaw: CharacterFlaw, val delta: Float) : SocialOp
    /** Bump a real [com.kursi.ai.persona.PersonaPolicy] grudge — the engine-facing teeth of Badla. */
    data class Grudge(val holder: Int, val target: Int, val weight: Int) : SocialOp

    companion object { const val ALL = -1 }
}
...
object StoryArcs {
    ...
    fun openingSuggestions(arc: ArcId, opponents: List<SeatRef>, flawOf: (Int) -> CharacterFlaw?): List<ChatSuggestion> =
        when (arc) {
            ArcId.GATHBANDHAN -> opponents.map {
                ChatSuggestion(
                    id = "start.gathbandhan.${it.seat}",
                    label = "Gathbandhan: ${it.name}",
                    kind = ChatActionKind.ARC_START, arc = arc, targetSeat = it.seat, targetName = it.name,
                    consequence = "Secret pact — coordinate, then betray",
                )
            }
            ArcId.AFWAAH -> opponents.map {
                ChatSuggestion(
     ...
     */
    fun begin(arc: ArcId, target: Int, targetName: String, accepted: Boolean): ArcStep = when (arc) {
        ArcId.GATHBANDHAN -> {
            val ops = if (accepted) listOf(SocialOp.Ally(0, target), SocialOp.Trust(target, 0, 0.5f)) else
            ...
            val beats = listOf(
                ...
                ArcBeat(target, if (accepted) "gathbandhan.accept" else "gathbandhan.decline",
                    if (accepted) MessageTone.FRIENDLY else MessageTone.HOSTILE, arc, 0),
            )
            val next = if (accepted) ArcState(arc, stage = 1, target = target, ally = target) else ArcState(arc, ended = true)
            val sugg = if (accepted) listOf(
                ChatSuggestion("badla.gathbandhan.knife.$target", "Knife $targetName first",
                    ChatActionKind.DEFLECT, arc, target, targetName, "Betray the pact before they do"),
            ) else emptyList()
            ArcStep(ops, beats, next, sugg)
        }
        ArcId.AFWAAH -> ArcStep(
            ops = listOf(SocialOp.Suspicion(SocialOp.ALL, target, 0.30f), SocialOp.Threat(target, 0.40f),
            ...
            suggestions = listOf(
                ChatSuggestion("afwaah.fuel.$target", "Aur hawa do (fuel it)", ChatActionKind.ARC_REPLY, arc, target, targetName,
                    "Twist the knife — more heat on $targetName"),
            ),
        )
        ArcId.STING -> ArcStep(
            ops = listOf(SocialOp.Agitate(target, CharacterFlaw.EGO, 0.6f), SocialOp.Agitate(target, CharacterFlaw.GREED, 0.5f),
            ...
            nextState = ArcState(arc, stage = 1, target = target),
            suggestions = listOf(
                ChatSuggestion("sting.dare.$target", "Dare them to prove it", ChatActionKind.ARC_REPLY, arc, target, targetName,
    ...
    fun reply(state: ArcState, input: HumanChatInput, rivalName: String? = null): ArcStep = when (state.arc) {
        ArcId.GATHBANDHAN -> when (input.kind) {
            ChatActionKind.DEFLECT -> ArcStep( // knife the ally first
        ...
        ArcId.BADLA -> { // input.targetSeat is the RIVAL to point the grudge at
            ...
            ArcStep(
                ops = listOf(SocialOp.Grudge(vengeful, rival, 3), SocialOp.Threat(rival, 0.4f),
