...
class ChatVoice(private val language: Language = Language.HINGLISH) {
    ...
    fun pileOn(personaId: String, targetName: String): String = when (personaId) {
    ...
    fun threatened(personaId: String): String = when (personaId) {
    ...
    fun taunt(personaId: String, targetName: String): String = when (personaId) {
    ...
    fun gloat(personaId: String): String = when (personaId) {
    ...
    fun flawBlurt(personaId: String, flaw: CharacterFlaw): String = when (flaw) {
    ...
    // ── Ally coordination ─────────────────────────────────────────────────────────
    fun allyWith(personaId: String, allyName: String): String =
    ...
    fun arcBeat(beatKey: String, speakerName: String, targetName: String?): String {
        val t = targetName ?: "unhe"
        return when (beatKey) {
            // GATHBANDHAN
            "gathbandhan.offer"   -> if (hi) "$t, sun. Tu-main milke khelein? Tu mujhe, main tujhe — abhi nahi." else "$t, listen. Shall we team up? You and me — not yet against each other."
    ...
    fun botArcBeat(beatKey: String, personaId: String, targetName: String?): String {
        val t = targetName ?: ""
        return when (beatKey) {
            "gathbandhan.accept"  -> when (personaId) {
            ...
            "gathbandhan.decline" -> when (personaId) {
