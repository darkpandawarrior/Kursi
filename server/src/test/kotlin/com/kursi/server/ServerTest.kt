...
class ServerTest {
    ...
    @Test
    fun `creating a room returns a non-blank room code`() = testApplication {
        application { module() }
     ...
     */
    @Test
    fun `two clients join a match and both receive StateUpdate`() = testApplication {
        application { module() }
     ...
     */
    @Test
    fun `StateUpdate never exposes hidden roles of other seats`() = testApplication {
        application { module() }
    ...
    @Test
    fun `out-of-turn intent returns Error ServerMessage and does not crash server`() = testApplication {
        application { module() }
    ...
    @Test
    fun `submitting a legal intent advances the game state`() = testApplication {
        application { module() }
        ...
        coroutineScope {
            ...
            val job1 = launch {
                wsClient.webSocket("/play") {
                    ...
                    val actorSeat = when (val ph = initView.phase) {
            ...
            val job2 = launch {
                ...
                wsClient.webSocket("/play") {
                    ...
                    val actorSeat = when (val ph = initView2.phase) {
                        is WirePhaseView.Turn -> ph.actor
                        else -> -1
                    }
    ...
    private fun chooseIntentFromView(view: WirePlayerView, seat: Int): WireIntent? {
        return when (val ph = view.phase) {
            ...
            is WirePhaseView.InfluenceLoss -> {
                ...
                val faceDown = view.myCards.firstOrNull { !it.faceUp }
                    ?: return null
    ...
    private fun assertSecrecy(view: WirePlayerView, seat: Int) {
    ...
    @Test
    fun `two clients play a full match to completion via the server`() = testApplication {
        application { module() }
        ...
        suspend fun runSeat(barrier: CompletableDeferred<Unit>?, awaitBarrier: CompletableDeferred<Unit>?) {
            wsClient.webSocket("/play") {
                sendMsg(ClientMessage.JoinRoom(matchId = roomCode, roomCode = roomCode))
    ...
    @Test
    fun `disconnected client can reconnect to its seat and resume with current state`() = testApplication {
        application { module() }
