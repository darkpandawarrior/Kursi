...
@Serializable
data class StandingDto(val position: Int, val name: String, val rating: Int)

/** Demo ladder — replaced by a persistent store once ratings are tracked. */
private val demoStandings: List<StandingDto> = listOf(
...
fun Application.configureRouting(registry: RoomRegistry) {
    routing {
        ...
        webSocket("/play") {
            ...
            val firstText = (incoming.receive() as? Frame.Text)?.readText()
                ?: run { close(CloseReason(CloseReason.Codes.VIOLATED_POLICY, "Expected JoinRoom")); return@webSocket }

            val joinMsg: ClientMessage = try {
            ...
            val actor = registry.findRoom(roomCode)
                ?: run {
                    ...
                    registry.createRoom(2).let { newCode ->
                        ...
                        val errMsg: ServerMessage = ServerMessage.Error(
                            matchId = roomCode,
                            seq = 0,
                            clientSeq = -1,
        ...
        post("/rooms/{playerCount}") {
            val playerCount = call.parameters["playerCount"]?.toIntOrNull()
                ?: run { call.respond(io.ktor.http.HttpStatusCode.BadRequest, "Invalid player count"); return@post }
