...
class TeamAwarenessTest {
    ...
    private fun policies(seedBase: Long, make: (Long) -> Policy): Map<PlayerId, Policy> =
        (0 until 4).associate { seat -> PlayerId(seat) to make(seedBase + seat) }

    private val builders: List<Pair<String, (Long) -> Policy>> = listOf(
    ...
    fun mixed_policy_team_game_completes() {
        ...
        val cfg = teamCfg4()
        val mixed: Map<PlayerId, Policy> = mapOf(
    ...
    private fun runDecide(p: Policy, view: PlayerView, legal: List<Intent>): Intent =
        p.decide(view, legal)
     ...
     */
    private fun stateWithCoins(cfg: GameConfig, seed: Long, actorSeat: Int, actorCoins: Int): GameState {
