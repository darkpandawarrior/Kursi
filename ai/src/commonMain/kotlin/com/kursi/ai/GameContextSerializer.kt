...
@Serializable
data class RankedAction(val action: String, val ismctsScore: Double)

object GameContextSerializer {

    private val json = Json { prettyPrint = false }

    fun serialize(view: PlayerView, ranked: List<MoveAdvice>): String {
        val opponents = view.players
        ...
        val candidates = ranked.take(5).map { advice ->
            RankedAction(
        ...
        val ctx = GameContext(
            coins = view.myCoins,
            influence = view.myInfluence.map { it.label() } + List(
    ...
    private fun describePhase(view: PlayerView): String {
        return view.phase.toString()
    }
    ...
    fun intentMatchesLabel(intent: Intent, label: String): Boolean {
